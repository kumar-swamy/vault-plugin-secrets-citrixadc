	...
	return &framework.Path{
		...
		Fields: map[string]*framework.FieldSchema{
			...
			"user_name": {
				...
				Description: "The username/logon for which this role will be associated.",
			},
			"ttl": {
...
func (b *backend) roleUpdateOperation(ctx context.Context, req *logical.Request, fieldData *framework.FieldData) (*logical.Response, error) {
	...
	userName, err := getUserName(fieldData)
	if err != nil {
		return nil, err
	}

	// verify service account exists
	_, err = b.client.Get(engineConf.ADCConf, userName)
	if err != nil {
		return nil, err
	}
...
func getUserName(fieldData *framework.FieldData) (string, error) {
	userName := fieldData.Get("user_name").(string)
	if userName == "" {
...
const (
	roleHelpSynopsis = `
Manage roles to build links between Vault and Citrix ADC user accounts.
`
	roleHelpDescription = `
...
To learn which usernames  are being managed by Vault, list the role names using
