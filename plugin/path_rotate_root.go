...
func (b *backend) pathRotateRootCredentialsUpdate(ctx context.Context, req *logical.Request, _ *framework.FieldData) (*logical.Response, error) {
	...
	oldPassword := engineConf.ADCConf.AdminPassword
	...
	if err := b.client.UpdateRootPassword(engineConf.ADCConf, engineConf.ADCConf.AdminUserName, newPassword); err != nil {
		return nil, err
	}
	engineConf.ADCConf.AdminPassword = newPassword
...
func (b *backend) rollBackRootPassword(ctx context.Context, engineConf *configuration, oldPassword string) error {
	...
	for i := 0; i < 10; i++ {
		...
		if err = b.client.UpdateRootPassword(engineConf.ADCConf, engineConf.ADCConf.AdminUserName, oldPassword); err == nil {
			// Success.
			return nil
		}
	}
