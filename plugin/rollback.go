...
func (b *backend) handleRotateCredentialRollback(ctx context.Context, storage logical.Storage, data interface{}) error {
	...
	if wal.CurrentPassword == "" {
		b.Logger().Warn("WAL does not contain a password for Username")
		return nil
	}
	...
	if err := b.client.UpdatePassword(conf.ADCConf, role.UserName, wal.CurrentPassword); err != nil {
		return err
	}
	username, err := getUsername(role.UserName)
	if err != nil {
		return err
	}
